	//변수 선언 및 초기화
	var nVideoInfoCnt = 0;
	var htVideoInfo = [];		//video file정보 저장
	var aResult = [];
	
	var rFilter = /^(video\/mp4)$/i;  
	var rFilter2 = /^(mp4)$/i; 
	var nTotalSize = 0;
	var nMaxVideoSize = 50*1024*1024;
	var nMaxTotalVideoSize = 50*1024*1024;
	var nMaxVideoCount = 1;
	var nVideoFileCount = 0;
	...
	var elCountTxtTxt = jindo.$$.getSingle("#videoCountTxt",elContent);
	 ...
	 */
     function goStartMode(){
    	 ...
    	 if(sSrc.indexOf("btn_confirm2.png") < 0 ){
    		 ...
    		 fnUploadVideo.attach(welBtnConfirm.$value(), "click");
    	 }
     } 
     /**
      * 동영상를 첨부 전 비활성화된 버튼 상태
      * @return
      */
     function goReadyMode(){
    	 var sSrc = welBtnConfirm.attr("src")|| "";
    	 if(sSrc.indexOf("btn_confirm2.png") >= 0 ){
    		 fnUploadVideo.detach(welBtnConfirm.$value(), "click");
    ...
    /** 
     * 동영상 첨부 전 안내 텍스트가 나오는 배경으로 '설정'하는 함수.
     * @return
     */
 	...
 	/**
 	 * 동영상 첨부 전 안내 텍스트가 나오는 배경을 '제거'하는 함수. 
 	 * @return
 	 */
	...
	//--------------------- html5  지원되는 브라우저에서 사용하는 함수  --------------------------
 	/**
 	 * 팝업에 노출될 업로드 예정 동영상의 수.
 	 * @param {Object} nCount 현재 업로드 예정인 동영상 장수
 	...
 	function updateViewCount (nCount, nVariable){
 		...
 		nVideoFileCount = nCnt;
 		return nCnt;
 	}
 	
 	/**
 	 * 팝업에 노출될 업로드될 동영상 총 용량
 	 */
 	 ...
 	 */
 	function refreshTotalVideoSize(sParentId){
 		var nDelImgSize = htVideoInfo[sParentId].size;
 	...
 	/**
 	 * hash table에서 동영상 정보 초기화.
 	 * @param {Object} sParentId
 	 */
 	function removeVideoInfo (sParentId){
 		//삭제된 동영상의 공간을 초기화 한다.
 		htVideoInfo[sParentId] = null;
 	}
 	
 	
 	/**
 	 * byte로 받은 동영상 용량을 화면에 표시를 위해 포맷팅
 	 * @param {Object} nByte
 	 */
 	function setUnitString (nByte) {
 		...
 		if( nByte < 1024) {
 			nVideoSize = Number(nByte);
 			sUnit = 'B';
 			return nVideoSize + sUnit;
 		} else if( nByte > (1024*1024)) {
 			nVideoSize = Number(parseInt((nByte || 0), 10) / (1024*1024));
 			sUnit = 'MB';
 			return nVideoSize.toFixed(2) + sUnit;
 		} else {
 			nVideoSize = Number(parseInt((nByte || 0), 10) / 1024);
 			sUnit = 'KB';
 			return nVideoSize.toFixed(0) + sUnit;
 		}
     }
 	...
 	/**
 	 * Total Video Size를 체크해서 추가로 동영상를 넣을지 말지를 결정함.
 	 * @param {Object} nByte
 	 */
 	function checkTotalVideoSize(nByte){
 		if( nTotalSize + nByte < nMaxTotalVideoSize){
    ...
    function drop(ev) {
		...
		if(typeof ev.dataTransfer.files == 'undefined'){
			...
			if (!!files && nCount === 0){
				//파일이 아닌, 웹페이지에서 동영상를 드래서 놓는 경우.
				alert("정상적인 첨부방식이 아닙니다.");
				return ;
			}
			
			for (var i = 0, j = nVideoFileCount ; i < nCount ; i++){
				if (!rFilter.test(files[i].type)) {
					alert("동영상파일 (mp4)만 업로드 가능합니다.");
				} else if(files[i].size > nMaxVideoSize){
					alert("동영상 용량이 10MB를 초과하여 등록할 수 없습니다.");
				} else {
					//제한된 수만 업로드 가능.
					if ( j < nMaxVideoCount ){
						sListTag += addVideo(files[i]);
						
						//다음 동영상을위한 셋팅
						j = j+1;
						nVideoInfoCnt = nVideoInfoCnt+1;
					} else {
						alert("최대 1장까지만 등록할 수 있습니다.");
						break;			
					}
			...
			if(j > 0){
				...
				//동영상 총사이즈 view update 
				updateViewTotalSize();
				//이미치 총 수 view update
				nVideoFileCount = j;
				updateViewCount(nVideoFileCount, 0);
				// 저장 버튼 활성화
				goStartMode();
			}else{
				readyModeBG();
    ...
    /**
     * 동영상를 추가하기 위해서 file을 저장하고, 목록에 보여주기 위해서 string을 만드는 함수.
     * @param ofile 한개의 동영상 파일
     * @return
     */
    function addVideo(ofile){
    	//파일 사이즈
		var ofile = ofile,
			sFileSize = 0,
			sFileName = "",
		...
		bExceedLimitTotalSize = checkTotalVideoSize(ofile.size);

		if( !!bExceedLimitTotalSize ){
			alert("전체 동영상 용량이 50MB를 초과하여 등록할 수 없습니다. \n\n (파일명 : "+sFileName+", 사이즈 : "+sFileSize+")");
		} else {
			//동영상 정보 저장							
			htVideoInfo['img'+nVideoInfoCnt] = ofile;
			
    		//List 마크업 생성하기
			aFileList.push('	<li id="img'+nVideoInfoCnt+'" class="imgLi"><span>'+ sFileName +'</span>');
			aFileList.push('	<em>'+ sFileSize +'</em>');
	        aFileList.push('	<a onclick="delVideo(\'img'+nVideoInfoCnt+'\')"><img class="del_button" src="./img/btn_del.png"  width="14" height="13" alt="첨부 동영상 삭제"></a>');
			aFileList.push('	</li> ');   
    ...
    /**
     * HTML5 DragAndDrop으로 동영상을 추가하고, 확인버튼을 누른 경우에 동작한다.
     * @return
     */
    function html5Upload() {	
		...
		sUploadURL= '/upload050'
    	
    	//파일을 하나씩 보내고, 결과를 받음.
    	for(var j=0, k=0; j < nVideoInfoCnt; j++) {
    		tempFile = htVideoInfo['img'+j];
    		try{
	    		if(!!tempFile){
    ...
    function callAjaxForHTML5 (tempFile, sUploadURL){
        ...
        return new Promise(
            (resolve, reject) => {
                ...
                file_upload_form.append("file", tempFile);
                
                xhr.send(file_upload_form);
                xhr.addEventListener('load', () => {
                    // const response = JSON.parse(xhr.responseText);
					// resolve(response);
					aResult.push({ sFileName : tempFile.name,
								 sFileURL : video_url,
								 bNewLine : true });
					//성공 시에  responseText를 가지고 array로 만드는 부분.
					// makeArrayFromString(returnText);
					if(aResult.length == nVideoFileCount){
		...
		// 				alert("동영상 파일(jpg,gif,png,bmp)만 업로드 하실 수 있습니다. ("+sFileName+")");
		// 			}else{
		// 				//성공 시에  responseText를 가지고 array로 만드는 부분.
    ...
    /**
 	 ...
 	 * @param {Object} sParentId 
 	 */
 	function delVideo (sParentId){
 		var elLi = jindo.$$.getSingle("#"+sParentId);
 		
 		refreshTotalVideoSize(sParentId);
 		
 		updateViewTotalSize();
 		updateViewCount(nVideoFileCount,-1);
 		//동영상 file array에서 정보 삭제.
 		removeVideoInfo(sParentId);
 		//해당 li삭제
 		$Element(elLi).leave();
 		
 		//마지막 동영상인경우.
 		if(nVideoFileCount === 0){
 			readyModeBG();
 			//동영상 추가 버튼 비활성화
 			goReadyMode();
 		}
	...
	function onAjaxError (){
		alert("[가이드]동영상 업로더할 서버URL셋팅이 필요합니다.-onAjaxError");
	}
...
// 	    		oCustomEvent.stop(); 수행시 bAllowed 가 false이더라도 alert이 수행되지 않음
	...
	window.onload = function(){
  		...
  		fnUploadVideo = $Fn(uploadVideo,this);
	 ...
	 */
 	function setPhotoToEditor(oFileInfo){
		if (!!opener && !!opener.nhn && !!opener.nhn.husky && !!opener.nhn.husky.PopUpManager) {
			...
			opener.nhn.husky.PopUpManager.setCallback(window, 'SET_VIDEO', [oFileInfo]);
